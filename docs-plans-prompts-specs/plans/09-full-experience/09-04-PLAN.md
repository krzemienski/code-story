...
<context>
@BRIEF.md
@src/codestory/api/routers/stories.py
@src/codestory/db/models.py
</context>
...
<task type="auto">
  ...
  <files>src/codestory/api/routers/sharing.py</files>
  <action>
...
```
  ...
  <verify>cd src/codestory && python -c "from api.routers.sharing import router; print('OK')"</verify>
  <done>Sharing API created</done>
</task>

<task type="auto">
  <n>Task 2: Create sharing UI components</n>
  <files>src/codestory/frontend/src/components/stories/ShareDialog.tsx</files>
  <action>
