...
<objective>
Implement S3-compatible cloud storage for audio file uploads, downloads, and presigned URL generation with SDK tool integration.
</objective>

<context>
@BRIEF.md
@src/codestory/backend/core/config.py
@src/codestory/backend/sdk/tools.py
</context>

<reasoning_guidance>
Storage is used by the SDK's synthesize_audio tool to upload generated audio files.

**SDK Integration:**
- mcp__codestory__synthesize_audio tool calls S3 upload after audio generation
- Audio URL is stored in story record and returned in completion event
- Presigned URLs provide secure, time-limited access

**Error Recovery Strategy:**
- Retry with exponential backoff (3 attempts)
- Multipart upload for files >100MB
- Validate file integrity with MD5/ETag
- Local fallback for failed uploads (queue for retry)
</reasoning_guidance>
...
<task type="auto">
  ...
  <files>src/codestory/backend/storage/s3.py, src/codestory/backend/storage/__init__.py</files>
  <action>
...
from src.codestory.backend.storage.s3 import (
    S3Storage,
    S3StorageError,
    storage,
    get_story_audio_key,
...
```
  ...
  <verify>python -c "from src.codestory.backend.storage.s3 import S3Storage; print('S3 OK')"</verify>
  <done>S3 storage client created</done>
</task>

<task type="auto">
  <n>Task 2: Create storage router</n>
  <files>src/codestory/backend/api/routers/storage.py</files>
  <action>
...
```
  ...
  <verify>python -c "from src.codestory.backend.api.routers.storage import router; print('Storage router OK')"</verify>
  <done>Storage router created</done>
</task>

<task type="auto">
  <n>Task 3: Add storage config settings</n>
  <files>src/codestory/backend/core/config.py</files>
  <action>
Add S3 settings to config (if not already present):
...
The config should use `pydantic-settings` to read from environment:
...
```
  ...
  <verify>python -c "from src.codestory.backend.core.config import get_settings; s = get_settings(); print(f'S3 bucket: {s.s3_bucket}')"</verify>
  <done>Storage config added</done>
</task>
