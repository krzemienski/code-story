...
<objective>
Implement environment configuration with pydantic-settings and integrate ClaudeAgentOptions patterns.

Purpose: Centralize all configuration in a type-safe, validated settings class that loads from environment variables and provides agent configuration.
Output: Working settings module with all API keys, database URLs, feature flags, and ClaudeAgentOptions integration.
</objective>
...
<task type="auto">
  ...
  <action>
    ...
    class Settings(BaseSettings):
        ...
        def get_claude_config(self) -> dict:
            """Get Claude Agent SDK configuration as dict.

            Returns config suitable for ClaudeAgentOptions.
            """
            return {
  ...
  <done>Comprehensive Settings class with type validation and ClaudeAgentOptions integration</done>
</task>
...
<task type="auto">
  ...
  <done>Core module exports and comprehensive .env.example with Claude Agent SDK config</done>
</task>
...
- [ ] Settings.get_claude_config() returns proper dict for ClaudeAgentOptions
...
- ClaudeAgentOptions configuration accessible via get_claude_config()
