...
<task type="auto">
  ...
  <action>
    ...
    - Add initial dependencies to pyproject.toml:
      - claude-agent-sdk (Claude Agent SDK - PRIMARY DEPENDENCY)
    ...
    IMPORTANT: Do NOT use `anthropic` directly. Use `claude-agent-sdk` which provides:
    - @tool decorator for creating MCP tools
    - create_sdk_mcp_server() for bundling tools into servers
  ...
  <done>Project initialized with claude-agent-sdk and all dependencies installed via uv</done>
</task>
...
<task type="auto">
  ...
  <action>
    ...
    │   └── codestory/
    │       ├── __init__.py
    │       ├── server.py            # MCP server using create_sdk_mcp_server()
    │       ├── agents/
    │       │   ├── __init__.py
    │       │   ├── base.py          # ClaudeAgentOptions configuration
    │       │   ├── intent.py        # Intent Agent (uses tools)
    │       │   ├── analyzer.py      # Repo Analyzer Agent
    │       │   ├── architect.py     # Story Architect Agent
    │       │   └── voice.py         # Voice Director Agent
    │       ├── tools/               # @tool decorated functions (NOT skills/)
    │       │   ├── __init__.py
    │       │   ├── github.py        # mcp__codestory__github_* tools
    │       │   ├── analysis.py      # mcp__codestory__analyze_* tools
    │       │   ├── narrative.py     # mcp__codestory__narrative_* tools
    │       │   └── voice.py         # mcp__codestory__voice_* tools
    │       ├── api/
    │       │   ├── __init__.py
    │       │   ├── main.py          # FastAPI app
    │       │   ├── routers/
    ...
    │       ├── core/
    │       │   ├── __init__.py
    │       │   ├── config.py        # Settings + ClaudeAgentOptions
    ...
    NOTE: Using `tools/` directory (not `skills/`) to match SDK pattern where @tool decorator
    creates MCP tools that Claude calls via mcp__<server>__<tool> naming.
...
- [ ] `uv run python -c "from claude_agent_sdk import tool, create_sdk_mcp_server"` succeeds
...
- Directory structure supports Claude Agent SDK architecture
- claude-agent-sdk is the primary Claude integration dependency
</success_criteria>
