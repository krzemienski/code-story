...
<context>
@BRIEF.md
@src/codestory/mobile/lib/api.ts
@src/codestory/backend/api/routers/storage.py
</context>
...
<task type="auto">
  ...
  <files>src/codestory/mobile/hooks/useAudioPlayer.ts</files>
  <action>
...
```
  ...
  <verify>cd src/codestory/mobile && npx tsc --noEmit</verify>
  <done>Audio player hook created</done>
</task>

<task type="auto">
  <n>Task 2: Create player screen with controls and chapter list</n>
  <files>src/codestory/mobile/app/story/[id]/play.tsx, src/codestory/mobile/components/PlayerControls.tsx</files>
  <action>
